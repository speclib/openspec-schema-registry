...
- [x] 1.1 Write `docs/registry-entry.md` with a field table covering `id`, `name`,
...
- [x] 1.2 Add a worked example to `docs/registry-entry.md`: the `speclib/tinychange` entry
...
- [x] 1.3 State in `docs/registry-entry.md` that the install destination is always
...
- [x] 1.4 State in `README.md` that the registry lists schemas you install, so the
...
- [x] 1.5 Link `docs/registry-entry.md` from `README.md` and `AGENTS.md` under a
...
- [x] 2.1 Write the entry that `speclib/openspec-tinychange-schema` would get and check
...
- [x] 2.2 Write the entries that a multi-schema repository would get, using
...
- [x] 2.3 Write the entry for `Lukk17/openspec-schemas`, the one candidate that publishes
...
- [x] 2.4 Write the entry for `danielhanold/superspec`, which declares `name: SuperSpec`
...
- [x] 2.5 Write a registry description of at most 100 characters for each schema in 2.1
      to 2.4, and verify none of them can be the upstream description unchanged.
- [x] 2.6 Record any field the cases above could not express in a short "findings"
...
- [x] 3.1 Add the entry model to `CHANGELOG.md` under `## [Unreleased]` / `### Added`,
      naming the capability rather than the file. Verify the entry follows the existing
      Keep a Changelog structure.
- [x] 3.2 Run `openspec validate define-registry-entry-model --strict` and verify it
